...
class selfsupervise_loss(nn.Module):
    ...
    def forward(self, attns, smooth=1e-40):
        ...
        for i in range(layer):
            ...
            # entropy loss
            log_attni = torch.log2(attni + smooth) #以2为底的指数
            entropy = -1 * torch.sum(attni * log_attni, dim=-1) / torch.log2(torch.tensor(n*1.0)) # b h n
            entropy_min = torch.min(entropy, dim=-1)[0]  # b h
            p_loss = (entropy_min-0.9).clamp_min(0)*(1/0.1)  ##torch.clamp_min_方法设置一个下限min，tensor中有元素小于这个值, 就把对应的值赋为min
            ...
            s_loss = self.smoothl1(attni*n, attni_t*n).clamp_min(0.1)  #torch.clamp_min_方法设置一个下限min，tensor中有元素小于这个值, 就把对应的值赋为min
